...
var listener_id = setlistener("sim/weight[2]/weight-lb" , func {
	setprop("controls/paratroopers/paratroopers", getprop("/sim/weight[2]/weight-lb") / 120)},  0, 0);
	...
	if(state.getValue()) {													# Si un parachutiste saute
		if (getprop("/instrumentation/doors/cargoramprear/position-norm") < 0.75) {						# Si la porte cargo n'est pas ouverte
			...
			var weight = getprop("/fdm/jsbsim/inertia/pointmass-weight-lbs[5]") - 240;							# On calcul le poids des parachutistes restant
			setprop("/fdm/jsbsim/inertia/pointmass-weight-lbs[5]", weight);									# On attribut le poids restant à la propriété
...
var listener_id2 = setlistener("sim/weight[3]/weight-lb" , func {
	setprop("controls/storage/storage", getprop("/sim/weight[3]/weight-lb") / storageWeight)}, 0, 0);
